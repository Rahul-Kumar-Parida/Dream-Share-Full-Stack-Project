...
class Post(Base):
    __tablename__ = 'posts'
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey('usershare.id'), nullable=False)
    content = Column(Text, nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow)
    owner = relationship('UserShare', back_populates='posts')
...
class Like(Base):
    ...
    created_at = Column(DateTime, default=datetime.utcnow)
    user = relationship('UserShare', back_populates='likes')
